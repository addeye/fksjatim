...
function del(){
	$id = $_GET['id'];
	$que = mysql_query("select * from tabel_koperasi WHERE id_koperasi = '$id' ");
	$d = mysql_fetch_array($que);
	$lokasifile = '../../images/'.$d['gambar_koperasi'];
	unlink($lokasifile);
	mysql_query("DELETE FROM `tabel_koperasi` WHERE `id_koperasi`='$id' ");
	// echo "lihat lah";
	header('Location:../?page=keanggotaan_tabel');
}
